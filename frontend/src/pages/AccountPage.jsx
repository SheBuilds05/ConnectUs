...
const AccountPage = () => {
  ...
  const [walletData, setWalletData] = useState({
    ...
    currency: 'ZAR',
    transactions: [
      { id: 1, type: 'credit', amount: 150.00, description: 'Refund - Booking #1234', date: '2026-03-01', status: 'completed' },
      { id: 2, type: 'debit', amount: 85.00, description: 'Payment to Lindiwe M.', date: '2026-02-28', status: 'completed' },
      { id: 3, type: 'credit', amount: 200.00, description: 'Wallet top-up', date: '2026-02-25', status: 'completed' },
      { id: 4, type: 'credit', amount: 45.00, description: 'Promo credit', date: '2026-02-20', status: 'completed' },
    ]
  });
  ...
  const stats = [
    { icon: <Package size={20} />, label: "Total Orders", value: "47" },
    { icon: <Award size={20} />, label: "Runners Helped", value: "12" },
    { icon: <Star size={20} />, label: "Average Rating", value: "4.9★" },
  ...
  const recentActivities = [
    ...
    { action: "Favorite runner Thandi N.", time: "1 week ago", status: "completed" }
  ];
  ...
  const handleWalletAction = (action) => {
    if (action === 'topup') {
      const amount = prompt('Enter amount to add to wallet (ZAR):', '100');
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5]">
      <div className="p-4 px-6 md:px-12 max-w-7xl mx-auto w-full space-y-8 pb-20">
        ...
        {/* Header with Working Back Button - OPTION 4 */}
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {stats.map((stat, index) => (
                <div key={index} className="bg-white rounded-3xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-all">
                  <div className="flex items-center gap-3 mb-2">
                    <div className="p-2 bg-green-50 text-[#2D531A] rounded-xl">
                      {stat.icon}
                    </div>
            ...
            <div className="relative bg-white rounded-[40px] p-8 border border-gray-100 shadow-md hover:shadow-lg transition-all">
              <div className="flex flex-col md:flex-row items-center gap-8">
                <div className="relative group">
                  <div className="w-32 h-32 bg-linear-to-br from-gray-50 to-gray-100 rounded-[35px] border-4 border-white shadow-xl flex items-center justify-center overflow-hidden">
                ...
                <div className="text-center md:text-left flex-1">
                  ...
                  <div className="flex flex-wrap gap-4 mt-3 text-sm text-gray-500">
                    <div className="flex items-center gap-1">
                      <Mail size={14} className="text-[#2D531A]" />
                      {isEditing ? (
                        <input
                          type="email"
                          name="email"
                    ...
                    <div className="flex items-center gap-1">
                      <Phone size={14} className="text-[#2D531A]" />
                      {isEditing ? (
                        <input
                          type="tel"
                          name="phone"
                ...
                {isEditing ? (
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="px-6 py-3 bg-[#2D531A] text-white font-bold rounded-2xl hover:bg-[#1a3a0f] flex items-center gap-2"
                    >
                      <Check size={16} />
                      Save
                    </button>
                    <button 
                      ...
                      className="px-6 py-3 bg-gray-100 text-gray-600 font-bold rounded-2xl hover:bg-gray-200 flex items-center gap-2"
                    >
                      <X size={16} />
                      Cancel
                    </button>
                  ...
                  <button 
                    ...
                    className="px-8 py-4 bg-[#2D531A] hover:bg-[#1a3a0f] text-white font-bold rounded-2xl transition-all shadow-md hover:shadow-lg flex items-center gap-2"
                  >
                    <Edit2 size={16} />
                    Edit Profile
                  </button>
        ...
        {selectedTab === 'wallet' && (
          <div className="space-y-6">
            {/* Wallet Balance Card */}
            <div className="bg-linear-to-br from-[#2D531A] to-[#0D330E] rounded-3xl p-8 text-white shadow-xl">
              ...
              <div className="mb-6">
                <span className="text-5xl font-black">R{walletData.balance.toFixed(2)}</span>
                <span className="text-white/60 ml-2">ZAR</span>
              </div>

              <div className="flex gap-4">
                <button className="flex-1 px-4 py-3 bg-white/10 hover:bg-white/20 rounded-xl text-sm font-bold transition-colors">
                  Withdraw
                </button>
                <button className="flex-1 px-4 py-3 bg-white/10 hover:bg-white/20 rounded-xl text-sm font-bold transition-colors">
                  Send to Bank
                ...
                <div className="bg-white rounded-3xl p-8 max-w-md w-full">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">Expiry</label>
                        <input
                          type="text"
                          placeholder="MM/YY"
                          className="w-full mt-1 px-4 py-3 bg-gray-50 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#2D531A] outline-none"
                    ...
                    <div className="flex gap-3 mt-6">
                      ...
                      <button 
                        ...
                        className="flex-1 px-6 py-4 bg-[#2D531A] text-white rounded-xl font-bold hover:bg-[#1a3a0f] transition-colors"
                      >
                        Add Card
                      </button>
                    </div>
